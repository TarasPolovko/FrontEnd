...
console.log('Type "skills" to see all skills');
console.log('Type "names" to see all the names');
console.log('Type "friends" to see all the friends names');

// get all skills from data
var skills = _.map(data, "skills");

// concat all uniq elements from skills into one array
var skills = _.union.apply(null, skills);

// order all skills without case sensitivity
var skills = _.orderBy(skills, [skill => skill.toLowerCase()]);

// order all names by number of friends 
var names = _.map(_.orderBy(data, [person => person.friends.length, "name"], ["desc", "asc"]), "name");

//get all array of friends
var friends = _.union.apply(null, _.map(data, "friends"));

// union all friends names into one array 
var friends = _.orderBy(_.union(_.map(friends, "name")));
